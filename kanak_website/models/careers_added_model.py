...
class Job(models.Model):
    ...
    vacant_location = fields.Many2many(
        'vacant.location', string="Vacancy Location")
    job_skill = fields.Many2many(
        'job.skills.require', string="Job Skills")
    ...
    def _compute_website_url(self):
        ...
        for job in self:
            job.website_url = slugify(job.name or '').strip().strip('-')
